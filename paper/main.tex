...
\vspace{-1.2em}
\begin{center}
\textbf{Project repository:}\quad\url{https://github.com/arassal/depth-project}
\end{center}
...
$0.8164$ vs.\ $0.8146$). Our methodological contribution is the
combined recipe itself, stabilized by a robust median-plus-MAD
normalization of the affine-invariant L1 loss; the hyperparameter
analysis isolates \emph{learning-rate-induced collapse} as the
dominant failure mode of small-data monocular depth distillation.
\end{abstract}
...
\textbf{Contributions.} Our technical contribution is the design of a
unified teacher-student training procedure for Depth Anything that
integrates two previously separate supervision signals, together with
a methodological change to the loss that is required for the
combination to be numerically stable:

\begin{enumerate}
    \item A single training pipeline that jointly applies
    \emph{output-level affine-invariant distillation} from a frozen
    Depth Anything Large teacher and a \emph{multi-scale gradient
    supervision} term against ground truth to a Depth Anything Small
    student. Neither prior work (Distill-Any-Depth~\cite{he2025distillanydepth},
    ZoeDepth~\cite{bhat2023zoedepth}) evaluated this combination, and
    neither was applied to the Depth Anything family
    (\cref{sec:approach}).
    \item A \emph{robust median-plus-MAD normalization} of the
    affine-invariant L1 loss in equation~\eqref{eq:robustnorm}, in
    place of the standard mean-plus-std formulation. The median/MAD
    variant is required to prevent the loss from exploding to
    \texttt{NaN} on NYU-v2 samples containing strong specular
    highlights -- a failure mode we observed in initial implementations
    and which the standard variant does not handle
    (\cref{sec:loss}).
    \item A unified evaluation methodology on two datasets that bound
    the relevant deployment regimes: NYU-v2 indoor (in-distribution
    fine-tuning) and KITTI eigen-test outdoor
    (out-of-distribution generalization). Both are evaluated under the
    same alignment procedure and at the same resolution, making the
    cross-domain comparison internally consistent
...
results, ablations, and a qualitative analysis. \cref{sec:conclusion}
...
discuss this interaction in \cref{sec:conclusion}.
...
We presented a unified open-source pipeline that combines
...
Anything Large teacher~\cite{yang2024depth}, stabilized by a
median-plus-MAD robust normalization of the affine-invariant L1 loss.
We evaluated on an NYU-v2 PoC subset and a KITTI eigen subset, performed
a controlled hyperparameter analysis over the distillation weight
$\alpha$, the gradient weight $\beta$, and the learning rate, and
produced an empirically tuned configuration that modestly improves the
...
The dominant variable in the hyperparameter analysis is the learning
rate, not the loss balance. The variance in test AbsRel within the
$\alpha$ sweep at fixed $\text{lr}=10^{-5}$ is approximately $0.09$,
covering only the difference between two collapse modes; the variance
within the learning-rate sweep at fixed $\alpha=0.7$ is approximately
$0.03$ but covers the difference between ``worse than zero-shot'' and
``better than zero-shot''. The recipe yields a small but consistent
gain only at $\text{lr}\le 10^{-6}$, where the student barely moves
from its pretrained weights; the ceiling for a recipe that does not
move the encoder is, by construction, the pretrained model itself.
Larger gains are expected from effective batch size $> 1$, the full
cross-context distillation variant of~\cite{he2025distillanydepth},
or more training images.

The principal limitations of the study are that all evaluation is at
$256{\times}256$ (which inflates absolute KITTI numbers but preserves
internal comparability), cross-domain generalization is tested on a
single OOD distribution, and we report a single seed per grid point;
the reproducibility of the collapse signature across configurations
suggests the qualitative finding is not seed-specific.
