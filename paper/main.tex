...
\tt\small aassal@cpp.edu
\and
Parsa Ghasemi\\
California State Polytechnic University, Pomona\\
Department of Electrical and Computer Engineering\\
\tt\small pghasemi@cpp.edu
}
...
\cref{tab:kitti} reports the cross-domain evaluation. The student
checkpoint trained on NYU-v2 indoor scenes does not transfer its
NYU-v2 improvement to KITTI outdoor scenes. Both AbsRel and $\delta_1$
are essentially unchanged ($\pm 0.01$ on AbsRel and $\pm 0.7$ percentage
points on $\delta_1$). We interpret this as a direct consequence of the
...
\textbf{Learning rate.} \cref{fig:lr} plots AbsRel and $\delta_1$ over
the learning rates $\{10^{-7}, 10^{-6}, 5{\cdot}10^{-6}, 10^{-5}\}$ at
fixed $\alpha=0.7$ and $\beta=0.1$. There is a sharp transition: at
...
set. Reading the per-image CSV files
...
\begin{figure}[t]
\centering
\includegraphics[width=0.49\columnwidth]{figs/loss_vs_epoch.png}
\includegraphics[width=0.49\columnwidth]{figs/val_absrel_vs_epoch.png}\\
\vspace{0.3em}
\includegraphics[width=0.49\columnwidth]{figs/val_delta1_vs_epoch.png}
\includegraphics[width=0.49\columnwidth]{figs/test_absrel_histogram.png}
\caption{\emph{Top:} training loss and validation AbsRel vs.\ epoch for
the default distillation configuration ($\alpha=0.5$, $\beta=0.1$,
$\text{lr}=10^{-5}$). Validation AbsRel collapses to $1.0$ at epoch
$2$, the signature of model output saturation. \emph{Bottom-left:}
validation $\delta_1$ over the same run; falls to $0.0$.
\emph{Bottom-right:} per-image AbsRel histogram on the NYU-v2 test
split for the zero-shot baseline.}
